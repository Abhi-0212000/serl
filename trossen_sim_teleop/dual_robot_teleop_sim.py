    ...
    def start_teleoperation(self, duration: float = 60.0):
        ...
        try:
            while time.time() < end_time and self.viewer.is_running():
                ...
                # Left robot (qpos 0-5: arm, 6-7: gripper)
                self.mj_data.qpos[0:6] = left_arm
                self.mj_data.qpos[6] = left_gripper   # left/right_carriage_joint
                self.mj_data.qpos[7] = left_gripper   # left/left_carriage_joint
                
                # Right robot (qpos 8-13: arm, 14-15: gripper)
                self.mj_data.qpos[8:14] = right_arm
                self.mj_data.qpos[14] = right_gripper  # right/right_carriage_joint
                self.mj_data.qpos[15] = right_gripper  # right/left_carriage_joint
